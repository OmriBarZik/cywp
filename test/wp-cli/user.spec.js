...
describe('User', () => {
  ...
  describe('#addCap()', () => {
    it('should have the arguments to add cap', () => {
      ...
      expect(user.wpUser).toHaveBeenLastCalledWith(['add-cap', 1, 'cap'])
    })
  })
  ...
  describe('#addRole()', () => {
    it('should have the arguments to add role', () => {
      ...
      expect(user.wpUser).toHaveBeenLastCalledWith(['add-role', 1, 'role'])
    })
  })
  ...
  describe('#create()', () => {
    describe('##throws', () => {
      it('should throw error if user name not provided', () => {
        expect(() => user.create()).toThrow(
          'options.userLogin must be provided!'
        )
      ...
      it('should throw error if password not provided', () => {
        expect(() => user.create({ userLogin: 'cywp' })).toThrow(
          'options.userPass must be provided!'
        )
      })
      ...
      it('should throw error if userRegistered is not a valid date', () => {
        expect(() =>
          user.create({
            userLogin: 'cywp',
            userPass: 'cywp',
            userRegistered: '2020.12.15'
          })
        ).toThrow('options.userRegistered must be instance of Date!')
      })
    })
    ...
    describe('##returns', () => {
      it('should have the arguments to create basic user', () => {
        ...
        expect(user.wpUser).toHaveBeenLastCalledWith([
          'create',
          '--porcelain',
          'cywp-user',
  ...
  describe('#list()', () => {
    ...
    it('should throw error when filters is not object', () => {
      ...
      expect(() => user.list('error')).toThrow(
  ...
  describe('#spam()', () => {
    it('should have the arguments to set user as spam', () => {
      ...
      expect(user.wpUser).toHaveBeenLastCalledWith(['spam', 'user'])
    })

    it('should have the arguments to set user as spam with user id as parameter', () => {
      user.spam(1)

      expect(user.wpUser).toHaveBeenLastCalledWith(['spam', '1'])
    })

    it('should have the arguments to set multiple user as spam', () => {
      user.spam(['user', 1])

      expect(user.wpUser).toHaveBeenLastCalledWith(['spam', 'user', 1])
    })
  })
  ...
  describe('#unspam()', () => {
    it('should have the arguments to unspam user', () => {
      ...
      expect(user.wpUser).toHaveBeenLastCalledWith(['unspam', 'user'])
    })

    it('should have the arguments to unspam user with user id as parameter', () => {
      user.unspam(1)

      expect(user.wpUser).toHaveBeenLastCalledWith(['unspam', '1'])
    })

    it('should have the arguments to unspam multiple users', () => {
      user.unspam(['user', 1])

      expect(user.wpUser).toHaveBeenLastCalledWith(['unspam', 'user', 1])
    })
  })
  ...
  describe('#update()', () => {
    it('should throw error when user id in not given', () => {
      expect(() => user.update({})).toThrow(
        new TypeError('options.user must be provided')
      )
    ...
    it('should throw error when user Registered date is not date object', () => {
      expect(() =>
        user.update({ user: 'user', userRegistered: 'error' })
      ).toThrow(
        new TypeError('options.userRegistered must be instance of Date!')
      )
    })
