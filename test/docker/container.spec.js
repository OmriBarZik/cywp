...
describe('Container', () => {
  ...
  describe('#start()', () => {
    it('should start docker container', async () => {
      ...
      return expect(container.start()).resolves.toMatchObject({
  ...
  describe('#stop()', () => {
    it('should stop the container', async () => {
      ...
      return expect(container.stop()).resolves.toMatchObject({
    ...
    it('should throw error when time smaller then 0', async () => {
      ...
      return expect(() => container.stop(-1)).toThrow(
        'time must be bigger then 0'
      )
    })
  })
    ...
    it('should return logs with time stamp', async () => {
      ...
      return container.logs({ timeStamps: true }).then((logs) => {
        expect(logs.stdout).toEqual(
          expect.stringContaining(time.toISOString().substr(0, 17))
        )
        expect(logs.stderr).toBeFalsy()
        expect(logs.container).toBe(container)
      })
  ...
  describe('#inspect()', () => {
    it('should return container info', async () => {
      ...
      return expect(container.inspect()).resolves.toMatchObject({
        Id: container.options.id,
    ...
    it('should return specific container info', async () => {
      ...
      return expect(container.inspect('{{.Id}}')).resolves.toBe(
  ...
  describe('#isHealthy()', () => {
    it('should return container health status', async () => {
      const container = await CreateContainer(
        { image: 'hello-world', health: { command: 'test -r ./hello' } },
        true
      )
    ...
    it("should throw error when health.command isn't defined", async () => {
      ...
      expect(() => container.isHealthy()).toThrow(
        'options.health.command must be defined to use IsHealthy'
      )
    })
  })
  ...
  describe('#exec()', () => {
    it('should have args to execute commands in docker', async () => {
      ...
      expect(container.dockerContainer.mock.calls[0][0]).toEqual([
        'exec',
        container.options.id,
        'command'
      ])
      expect(container.dockerContainer.mock.calls[0][1]()).toHaveProperty(
        'stdout'
      )
      expect(container.dockerContainer.mock.calls[0][1]()).toHaveProperty(
        'stderr'
