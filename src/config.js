...
function setWordpressVersion(wordpressVersion) {
  ...
  if (!validateVersion(wordpressVersion)) {
    throw new Error(
      "invalid wordpress version! please check 'wordpressVersion' and try again."
    )
  }
...
function setWordpressPort(wordpressPort) {
  ...
  if (!Number.isInteger(+wordpressPort)) {
    throw new Error(
      "invalid wordpress port! please check 'wordpressPort' and try again"
    )
  }
...
function SetWordpressThemeVersion(themeVersion) {
  ...
  if (!validateVersion(themeVersion)) {
    throw new Error(
      "invalid theme version! please check 'wordpressThemeVersion' and try again."
    )
  }
...
function setWordpressThemePath(wordpressThemePath, wordpressTheme) {
  ...
  if (!existsSync(wordpressThemePath)) {
    throw new Error(
      "invalid theme path! please check 'wordpressThemePath' and try again."
    )
  }

  return [
    {
      host: resolve(wordpressThemePath),
      docker: `/var/www/html/wp-content/themes/${wordpressTheme}:ro`
    }
  ]
}
  ...
  for (const plugin in wordpressPlugins) {
    ...
    if (!existsSync(pluginData)) {
      if (!validateVersion(pluginData)) {
        throw new Error(
          `invalid plugin version! please check "wordpressPlugins": {"${plugin}": "${pluginData}"} and try again.`
        )
      }
    ...
    if (!existsSync(pluginFile)) {
      throw new Error(
        `${plugin}.php is not found in ${pluginDir}. pleas provide a valid plugin path and try again.`
      )
    }

    const re = new RegExp(
      `Plugin Name:.*(${plugin}|${plugin.replace(/-/, ' ')}|${plugin.replace(
        / /,
        '-'
      )})`,
      'ig' // eslint-disable-line spellcheck/spell-checker
    )

    if (!readFileSync(pluginFile).toString().match(re)) {
      throw new Error(
        `${pluginFile} must be a valid plugin, please note that the plugin name matches the cypress.json config.`
      )
    }

    plugins.local.push({
      host: pluginDir,
      docker: `/var/www/html/wp-content/plugins/${plugin}:ro`,
      name: plugin
    })
  }
...
function checkConfig(config, quiet) {
  ...
  const cywpConfig = {
    ...
    cywpThemeVersion: SetWordpressThemeVersion(
      configJson.wordpressThemeVersion
    ),
    cywpPlugins: setWordpressPlugins(configJson.wordpressPlugins)
  }

  cywpConfig.cywpThemePath = setWordpressThemePath(
    configJson.wordpressThemePath,
    cywpConfig.cywpTheme
  )
